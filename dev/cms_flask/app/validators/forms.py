...
from wtforms import DateTimeField, PasswordField, FieldList, IntegerField, StringField, RadioField
...
class CreateOrUpdatePlatForm(Form):
    platform_name = StringField(validators=[DataRequired(message='必须传入平台名')])
    remark = StringField(validators=[DataRequired(message='必须传入备注')])
    is_disable = StringField(validators=[DataRequired(message='必须传入是否禁用')])